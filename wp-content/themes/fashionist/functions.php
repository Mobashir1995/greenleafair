...
if( !function_exists( 'fashionist_ajax_cart' ) ) {
    function fashionist_ajax_cart(){
        ...
        if(fashionist_checkPlugin('woocommerce/woocommerce.php') ):?>
            <span id="open-close-cart" onclick="open_close_dropdown('shopping-cart')"><span class="icon-shoppingcart"></span> ( <?php $k = ''; 
            $k = fashionist_woocommerce_header_add_to_cart_fragment($a); 
             echo wp_kses_post( $k['span.cart-count'] );  

            echo $fragments['span.cart-count'] ?>
         )
     </span>

            <?php
                 $k = ''; $k = fashionist_woocommerce_header_add_to_cart_fragment($a);
                if( $k['span.cart-count'] != "0" ) { ?>
                    <div id="shopping-cart" class="closed"><div class="cont"><span class="title">Shopping cart (<?php $k = ''; $k = fashionist_woocommerce_header_add_to_cart_fragment($k);  echo wp_kses_post( $k['span.cart-count'] ); ?>)</span><div class="widget_shopping_cart_content"></div></div></div>
                <?php } else { ?>
...
/*==============add to cart custom===========*/
add_action( 'admin_menu', 'address_option_register' );

function address_option_register()
{
    add_menu_page(
        'Site Addresses',     // page title
        'Site Addresses',     // menu title
        'manage_options',   // capability
        'site-addresses',     // menu slug
        'address_option_render' // callback function
    );
}
function address_option_render()
{
    global $title;
    if(isset($_POST['huston_submit'])){
    	if ( get_option( 'hustion_address' ) !== false ) {
    		update_option( 'hustion_address', $_POST['hustion_address'], '', 'yes' );
    		update_option( 'hustion_phone', $_POST['hustion_phone'], '', 'yes' );
    	}else{
    		add_option( 'hustion_address', $_POST['hustion_address'], '', 'yes' );
    		add_option( 'hustion_phone', $_POST['hustion_phone'], '', 'yes' );
    	}
    }
    if(isset($_POST['austin_submit'])){
    	if ( get_option( 'austin_address' ) !== false ) {
    		update_option( 'austin_address', $_POST['austin_address'], '', 'yes' );
    		update_option( 'austin_phone', $_POST['austin_phone'], '', 'yes' );
    	}else{
    		add_option( 'austin_address', $_POST['austin_address'], '', 'yes' );
    		add_option( 'austin_phone', $_POST['austin_phone'], '', 'yes' );
    	}
    }
    if(isset($_POST['san-antino_submit'])){
    	if ( get_option( 'san_antino_address' ) !== false ) {
    		update_option( 'san_antino_address', $_POST['san_antino_address'], '', 'yes' );
    		update_option( 'san_antino_austin_phone', $_POST['san_antino_austin_phone'], '', 'yes' );
    	}else{
    		add_option( 'san_antino_address', $_POST['san_antino_address'], '', 'yes' );
    		add_option( 'san_antino_austin_phone', $_POST['san_antino_austin_phone'], '', 'yes' );
    	}
    }
    $get_austion_add = get_option('austin_address');
    $get_austion_ph = get_option('austin_phone');

    $get_hustion_add = get_option('hustion_address');;
    $get_hustion_ph = get_option('hustion_phone');;

    $get_san_antino_add = get_option('san_antino_address');;
    $get_san_antino_ph = get_option('san_antino_austin_phone');;
    
    $rawHtml  = '<div class="wrap wp-core-ui">';
    $rawHtml .= "<h1>$title</h1>";
    $rawHtml .= "<ul class='address-list'>";
    $rawHtml .= "<li  class='";
    if($_GET['address'] == 'huston' || $_GET['address'] == ''){ 
     $rawHtml .=  "active";
    }
    $rawHtml .=  "'><h4><a href='https://www.greenleafair.com/wp-admin/admin.php?page=site-addresses&address=huston'>Houston</a></h4></li>";
    $rawHtml .= "<li  class='";
    if($_GET['address'] == 'austin'){ 
     $rawHtml .=  "active";
    }
    $rawHtml .=  "'><h4><a href='https://www.greenleafair.com/wp-admin/admin.php?page=site-addresses&address=austin'>Austin</a></h4></li>";
    $rawHtml .= "<li  class='";
    if($_GET['address'] == 'san-antino'){ 
     $rawHtml .=  "active";
    }
    $rawHtml .=  "'><h4><a href='https://www.greenleafair.com/wp-admin/admin.php?page=site-addresses&address=san-antino'>San Antonio</a></h4></li>";
    $rawHtml .= "</ul>";
    if($_GET['address'] == 'huston' || $_GET['address'] == ''){
           $rawHtml .= "<form method='post'><div class='test-input'><span class='input-title'>Address:</span><input name='hustion_address' value='".$get_hustion_add."' type='text'></div>";
           $rawHtml .= "<div class='test-input'><span class='input-title'>Mobile:</span><input name='hustion_phone' type='tel' value='".$get_hustion_ph."'></div>";
           $rawHtml .= "<div class='test-input'><input class='button button-primary' value='submit' name='huston_submit' type='submit'></div></form>";
    }
    if ($_GET['address'] == 'austin') {
    	    $rawHtml .= "<form method='post'><div class='test-input'><span class='input-title'>Address:</span><input name='austin_address' value='".$get_austion_add."' type='text'></div>";
           	$rawHtml .= "<div class='test-input'><span class='input-title'>Mobile:</span><input name='austin_phone' type='tel' value='".$get_austion_ph."'></div>";
           	$rawHtml .= "<div class='test-input'><input class='button button-primary' value='submit' name='austin_submit' type='submit'></div></form>";
    }
    if ($_GET['address'] == 'san-antino') {
    	    $rawHtml .= "<form method='post'><div class='test-input'><span class='input-title'>Address:</span><input name='san_antino_address' type='text' value='".$get_san_antino_add."'></div>";
           	$rawHtml .= "<div class='test-input'><span class='input-title'>Mobile:</span><input name='san_antino_austin_phone' type='tel' value='".$get_san_antino_ph."'></div>";
            $rawHtml .= "<div class='test-input'><input class='button button-primary' value='submit' name='san-antino_submit' type='submit'></div></form>";
    }
    $rawHtml .= '</div>';
    print $rawHtml;
}
...
function site_address(){
	ob_start();
	 $publicIP = get_client_ip(); 
    	$LocationArray = json_decode( file_get_contents('http://ip2location-api.com/api/json/'.$publicIP), true);
    	$get_austion_add = get_option('austin_address');
    	...
    	if($LocationArray['city'] == 'Austin'){
    		echo $get_austion_add;
    	}elseif ($LocationArray['city'] == 'Houston') {
    		echo $get_hustion_add;
    	}elseif ($LocationArray['city'] == 'San Antonio') {
    		echo $get_san_antino_add;
    	}else{
    		echo 'Green Leaf Air<br> 1168 Commerce Dr Richardson, TX 75081';
    	}
    	return ob_get_clean();
}
...
function site_address_map($atts){
		ob_start();
		$publicIP = get_client_ip(); 
		$height = $atts['height'];
    	$LocationArray = json_decode( file_get_contents('http://ip2location-api.com/api/json/'.$publicIP), true);
	    ...
	    $get_san_antino_add = get_option('san_antino_address');;
    	if($LocationArray['city'] == 'Austin'){
    		echo '<iframe width="100%" height="'.$height.'" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.it/maps?q='.$get_austion_add.'&output=embed"></iframe>';
    	}elseif ($LocationArray['city'] == 'Houston') {
    		echo '<iframe width="100%" height="'.$height.'" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.it/maps?q='.$get_hustion_add.'&output=embed"></iframe>';
    	}elseif ($LocationArray['city'] == 'San Antonio') {
    		echo '<iframe width="100%" height="'.$height.'" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.it/maps?q='.$get_san_antino_add.'&output=embed"></iframe>';
    	}else{
    		echo '<iframe width="100%" height="'.$height.'" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.it/maps?q=Green Leaf Air 1168 Commerce Dr Richardson, TX 75081&output=embed"></iframe>';
    	}
    	return ob_get_clean();
}
